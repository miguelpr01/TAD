...
<section class="section-lista-proteinas" id="proteinas">
    <div class="container">
        <h1 class="text-center mb-5">Creatinas</h1>
        @if (session('mensaje'))
        ...
        <div class="row">
            @foreach ($creatinas as $creatina)
                <div class="col-md-4 mb-4">
                    <div class="card">
                    <img src="{{ $productos[$creatina->producto_id]->imagen }}" class="card-img-top" alt="...">
                        <div class="card-body">
                            <h5 class="card-title">{{ $productos[$creatina->producto_id]->nombre }}</h5>
                            <p class="card-text">{{ $productos[$creatina->producto_id]->precio }}</p>
                            <a href="#" class="btn btn-success btn-block">Ver producto</a>
                            <form action="{{ route('editar_creatina', $productos[$creatina->producto_id]->id) }}" method="GET">
                                <button type="submit" class="btn btn-success btn-block">Editar</button>
                            </form>
                            <form action="{{ route('borrar_producto', $productos[$creatina->producto_id]->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
